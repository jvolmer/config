...
#+begin_src emacs-lisp
  ...
  (use-package lsp-mode
    ...
    (lsp-signature-render-documentation nil)  ;; show only signature, not documention when using function
...
#+begin_src emacs-lisp
  ...
  (use-package rustic
    :custom
    ;; make rust-analyzer work with ESP projects
    (rustic-analyzer-command (list (substring (shell-command-to-string "rustup which --toolchain stable rust-analyzer") 0 -1))))
...
| C-c C-c C-r | Run main  |
| C-c C-c C-t | Run tests |
