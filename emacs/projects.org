...
** Revisions
How to see two revisions side-by-side:
Normally magit reuses revision buffers, if you want to keep a revision buffer, execute =magit-toggle-buffer-lock= on it, then load the other revision.
...
| S | Save rg-buffer to a defined name buffer                              |
