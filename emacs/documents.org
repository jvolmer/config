...
#+begin_src emacs-lisp
  ...
  (use-package org
    ...
    (org-id-link-to-org-use-id t) ;; prefer id for linking
...
- Delete current column =org-table-delete-column=
- Re-align table ~C-c C-c~

*** Use as spreadsheet
In result field, type =:=<function>(@I..@II)= for values in same column between first and second vertical line, function could be e.g. =vmean=, this adds formular at end of table
One formula for all columns: =#+TBLFM: @4$2..@4$>=vmean(@I..@II)=
Rounding to 2 decimal digits: =#+TBLFM: @4$2..@4$>=vmean(@I..@II);%.2f=
Referencing other values: https://orgmode.org/manual/References.html#References

*** Plot
Ascii-plot of current column values as bar-plot in additional column =orgtbl-ascii-plot=
Gnuplot also possible: https://orgmode.org/manual/Org-Plot.html
...
*** Show hidden elements in insert mode
...
Write <s TAB for a generic source block
...
#+begin_src emacs-lisp
  ...
  (defun jv/org-babel-tangle-config (directory-or-file-name-list)
    (dolist (directory-or-file-name directory-or-file-name-list)
      (when (string-prefix-p (expand-file-name directory-or-file-name)
                             buffer-file-name)
        (let ((org-confirm-babel-evaluate nil))
          (org-babel-tangle)))))

  ;; this hook is loaded for each org-mode buffer
  (add-hook 'org-mode-hook (lambda () (add-hook 'after-save-hook (lambda () (jv/org-babel-tangle-config '("~/config/"))))))
...
#+begin_src emacs-lisp
  ...
  (use-package org-present
    :config (setq org-present-text-scale 3)
            (setq org-image-actual-width nil) ;; you can now define img width with preceeding #+ATTR_ORG: :width 1000
    :bind (:map org-present-mode-keymap
           ;; < and > should not change slides, also in insert-mode
           ("<right>" . evil-forward-char)
           ("<left>" . evil-backward-char)))
           ;; TODO set <normal state> r to nil
