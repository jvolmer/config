...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'org
    (setq org-agenda-files
          '("~/org/journal.org" "~/org/org-agenda/Tasks.org"))
...
! in brackets after short version adds time-logging when state changes into the specific state, @ adds a note, @/! adds both

#+begin_src emacs-lisp
  
  (with-eval-after-load 'org
    (setq org-todo-keywords
          '((sequence "WAIT(w!)" "TODO(t)" "NEXT(n)" "DOING(n)" "|" "DONE(d)")
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'org
    ...
    (setq org-log-into-drawer t)) ;; fold annotations (e.g. state changes) in collapsable drawer
...
Add filetags with ~org-roam-tag-add~
