...
| s          | replace character under cursor and stay in insert (substitute) |
...
there is some weird thing happening with my cursor when reverting

*** old comments package
does not work in presenting mode of =org-present=
#+begin_src emacs-lisp
